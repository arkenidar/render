...
char *starts_with(const char *string, const char *what)
{
  ...
  for (;; string_cursor++, what_cursor++)
  {
    ...
    if (*what_cursor != '\0' && *string_cursor == '\0')
      return NULL;
    if (*string_cursor != *what_cursor)
      return NULL;
  }
  return string_cursor;
}
...
*/
model load_model_obj(const char *file_name)
{ // "main.c" -> "parse.h" -> "head.obj"

  puts(file_name);

  model loaded;
  loaded.mesh.array = NULL;
  loaded.mesh.count = 0;
  loaded.vertex_positions.array = NULL;
  loaded.vertex_positions.count = 0;
  loaded.vertex_normals.array = NULL;
  loaded.vertex_normals.count = 0;
  ...
  while (fgets(line, 1001, file))
  {
    ...
    else if ((next = starts_with(line, "vn ")))
    {
      prefix = (char *)"vn ";
      if (print)
        printf("FOUND: %s%s", prefix, next);
    ...
    else if ((next = starts_with(line, "f ")))
    {
      prefix = (char *)"f ";
      if (print)
        printf("FOUND: %s%s", prefix, next);
